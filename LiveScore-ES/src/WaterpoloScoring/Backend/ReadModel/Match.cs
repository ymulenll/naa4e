...
using WaterpoloScoring.Framework.Utils;

namespace WaterpoloScoring.Backend.ReadModel
{
    public class Match
    {
        private Score _lastScore;
        private readonly Score[] _scorePartials = {new Score(), new Score(), new Score(), new Score()};
        ...
        {
            _scorePartials[CurrentPeriod - 1] = new Score(
                CurrentScore.TotalGoals1 - _lastScore.TotalGoals1,
                CurrentScore.TotalGoals2 - _lastScore.TotalGoals2);
        ...
        public Score GetPartial(int period)
        {
            if (period<1 || period >4)
                return new Score();
            return _scorePartials[period - 1];
        }
        #endregion
